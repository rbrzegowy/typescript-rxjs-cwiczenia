...
// Wypisz w konsoli/na  ekranie: Głosowanie rozpoczęte -> Głosowanie zakończone (przez 2s) -> Wyniki głosowania: ''
...
const orders$ = of('brak')
  .pipe(writeToHtml('#orders', { customValueFn: order => JSON.stringify(order), add: 'beforeend' }))
  .subscribe()
...
const pln$ = interval(1_200).pipe(take(plns.length), map(i => plns[i]))
const eurRate$ = interval(1_400).pipe(take(eurRates.length), map(i => eurRates[i]), share())
...
const media$ = of('wyłączony')
  .pipe(
    writeToHtml('#media-player-state')
  )
  .subscribe()
