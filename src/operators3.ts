...
function uploadIt(url: string, data: string) {
  return of(Math.random() > 0.5 ? `OK  ${data}` : `Bad request ${data}`).pipe(
...
// nie możemy założyć że wszystkie dane mamy od początku
setTimeout(() => {
...
// Wersja standard: jeden zapis na raz (poprzedni trwający jest anulowany)
// BONUS: dopuszczamy równoległe zapisy
